...
public class EnemmyMovement : MonoBehaviour
{
    ...
    {
        // Random MoveSpeed
        moveSpeedX = Random.Range(0.5f, 1.5f);
        moveSpeedZ = Random.Range(0.5f, 1.5f);
        moveRight = true;
        moveDown = true;
    ...
    private void LookAtPlayer()
    {
        ...
        // Calculate the direction vector from the enemy to the player, ignoring the y-axis difference
        Vector3 directionToPlayer = transform.position - playerTransform.position; // Inverted direction
        directionToPlayer.y = 0; // Keep the rotation only in the horizontal plane

        // Create a rotation that looks along the directionToPlayer
        Quaternion lookRotation = Quaternion.LookRotation(directionToPlayer);

        // Apply the rotation to the enemy
        transform.rotation = Quaternion.Slerp(transform.rotation, lookRotation, Time.deltaTime * 3.0f); // Smooth rotation, adjust the speed as needed
    }
